...
int main(int argc,char **argv){
	...
	long int time1[720];//experiment2
	long int time2[720];//experiment2
	long int size[720];//experiment2
	int number=0;//experiment2
	...
	struct  timeval    tv1;//experiment2SCTP
	struct  timezone   tz1;//experiment2SCTP
	int ii=0;//experiment2SCTP
...
//-----------------------------------------------------------
		if (FD_ISSET(sctpsock_id,&fd) ){
			if( ii == 0 ){
				gettimeofday(&tv1,&tz1);//experiment2SCTP
				ii=1;
			}
		...
		if (FD_ISSET(connSock,&fd) ){
			...
			if(recv_len == 0){
				...
				gettimeofday(&tv,&tz);//experiment2
				time1[number]=tv.tv_sec;//experiment2
				time2[number]=tv.tv_usec;//experiment2
				if( number == 0 ){//experiment2
					size[number]=write_len;//experiment2
				}else{//experiment2
					size[number]=size[number-1]+write_len;//experiment2
				}//experiment2
				number++;//experiment2
...
//-----------------------------------------------------------
	...
	printf("first time:%ld %ld\n",time1[0],time2[0]);//experiment2
	for(i=1;i<number;i++){//experiment2
		if(time1[i]>time1[0] && time2[i]<time2[0]){
			...
			}else{printf("%ld.%ld %d\n",time1[i]-1-time1[0],time2[i]+1000000-time2[0],size[i]);//experiment2
			}
		}else{
			...
			}else{printf("%ld.%ld %d\n",time1[i]-time1[0],time2[i]-time2[0],size[i]);//experiment2
			}
		}
	}//experiment2
	...
	printf("us:%ld um:%ld\n", firststss, firststsm);//experiment2SCTP
	gettimeofday(&tv,&tz);//experiment2SCTP
	printf("ct:%ld cm:%ld\n", tv.tv_sec, tv.tv_usec);//experiment2SCTP
	if (tv.tv_sec > firststss && tv.tv_usec < firststsm){//experiment2SCTP
		printf("delivery time:%ld.%ld\n", tv.tv_sec-1-firststss, tv.tv_usec+1000000-firststsm);//experiment2SCTP
	}else{//experiment2SCTP
		printf("delivery time:%ld.%ld\n", tv.tv_sec-firststss, tv.tv_usec-firststsm);//experiment2SCTP
	}//experiment2SCTP

	if (tv.tv_sec > tv1.tv_sec && tv.tv_usec < tv1.tv_usec){//experiment2SCTP
		printf("receiving time:%ld.%ld\n", tv.tv_sec-1-tv1.tv_sec, tv.tv_usec+1000000-tv1.tv_usec);//experiment2SCTP
	}else{//experiment2SCTP
		printf("receiving time:%ld.%ld\n", tv.tv_sec-tv1.tv_sec, tv.tv_usec-tv1.tv_usec);//experiment2SCTP
	}//experiment2SCTP
