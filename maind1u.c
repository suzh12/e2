...
int main(int argc,char **argv){
	...
	struct  timeval    tv1;//experiment2UDP
	struct  timezone   tz1;//experiment2UDP
	int ii=0;//experiment2UDP
...
//-----------------------------------------------------------
		if (FD_ISSET(udpsock_id,&fd)){
			if ( ii == 0 ){
				gettimeofday(&tv1,&tz1);//experiment2UDP
				ii=1;
			}
			if (udpfinish==0){
				...
				if( number == 0 ){//experiment2
					size[number]=write_length;//experimnt2
				}else{//experiment2
					size[number]=size[number-1]+write_length;//experiment2
				}//experiment2
				number++;//experiment2
...
//-----------------------------------------------------------
	...
	printf("us:%ld um:%ld\n", firstutss, firstutsm);//experiment2UDP
	gettimeofday(&tv,&tz);//experiment2UDP
	printf("ct:%ld cm:%ld\n", tv.tv_sec, tv.tv_usec);//experiment2UDP
	if (tv.tv_sec > firstutss && tv.tv_usec < firstutsm){//experiment2UDP
		printf("delivery time:%ld.%ld\n", tv.tv_sec-1-firstutss, tv.tv_usec+1000000-firstutsm);//experiment2UDP
	}else{//experiment2UDP
		printf("delivery time:%ld.%ld\n", tv.tv_sec-firstutss, tv.tv_usec-firstutsm);//experiment2UDP
	}//experiment2UDP

	if (tv.tv_sec > tv1.tv_sec && tv.tv_usec < tv1.tv_usec){//experiment2UDP
		printf("receiving time:%ld.%ld\n", tv.tv_sec-1-tv1.tv_sec, tv.tv_usec+1000000-tv1.tv_usec);//experiment2UDP
	}else{//experiment2UDP
		printf("receiving time:%ld.%ld\n", tv.tv_sec-tv1.tv_sec, tv.tv_usec-tv1.tv_usec);//experiment2UDP
	}//experiment2UDP
