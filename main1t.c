...
void *thread1(void *argc) 
{
	...
	commonseq=1;
	while ((read_len = fread(data->data, sizeof(char), SLAdata, fp)) >0 ) {
		if ( (commonseq%socknum) == 0 || (commonseq%socknum) == 1 || (commonseq%socknum) == 2 ){
...
int main(int argc,char **argv)
{
	...
	struct  timeval    tv;//experiment2
	struct  timezone   tz;//experiment2
	...
	printf("start time:%ld %ld\n",tv.tv_sec,tv.tv_usec);//experiment2
