...
int main(int argc,char **argv){
	...
	struct  timeval    tv1;//experiment1TCP
	struct  timezone   tz1;//experiment1TCP
	int ii=0;
...
//-----------------------------------------------------------
		if (FD_ISSET(tcpsock_id,&fd) ){//link_id//tcpsock_id
			if ( ii == 0 ){
				gettimeofday(&tv1,&tz1);//experiment1TCP
				ii=1;
			}
		...
		if (FD_ISSET(link_id,&fd) ){
			...
			if(recv_len == 0){
				...
				if( number == 0 ){//experiment2
					size[number]=write_leng;//experiment2
				}else{//experiment2
					size[number]=size[number-1]+write_leng;//experiment2
				}//experiment2
				number++;//experiment2	
...
//-----------------------------------------------------------
	...
	printf("us:%ld um:%ld\n", firstttss, firstttsm);//experiment1TCP
	gettimeofday(&tv,&tz);//experiment1TCP
	printf("ct:%ld cm:%ld\n", tv.tv_sec, tv.tv_usec);//experiment1TCP
	if (tv.tv_sec > firstttss && tv.tv_usec < firstttsm){//experiment1TCP
		printf("delivery time:%ld.%ld\n", tv.tv_sec-1-firstttss, tv.tv_usec+1000000-firstttsm);//experiment1TCP
	}else{//experiment1TCP
		printf("delivery time:%ld.%ld\n", tv.tv_sec-firstttss, tv.tv_usec-firstttsm);//experiment1TCP
	}//experiment1TCP

	if (tv.tv_sec > tv1.tv_sec && tv.tv_usec < tv1.tv_usec){//experiment1TCP
		printf("receiving time:%ld.%ld\n", tv.tv_sec-1-tv1.tv_sec, tv.tv_usec+1000000-tv1.tv_usec);//experiment1TCP
	}else{//experiment1TCP
		printf("receiving time:%ld.%ld\n", tv.tv_sec-tv.tv_sec, tv.tv_usec-tv1.tv_usec);//experiment1TCP
	}//experiment1TCP
